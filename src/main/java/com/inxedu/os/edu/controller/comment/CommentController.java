...
public class CommentController extends BaseController {
	...
	public Map<String, Object> addComment(HttpServletRequest request, Comment comment) {
		...
		try {
			...
			int sysUserId = SingletonLoginUtils.getLoginSysUserId(request);
			if (userId == 0) {
				if(sysUserId == 0){
					...
					// 系统用户id
					comment.setUserId(sysUserId);
					comment.setUserType(1);
					// 添加评论
					commentService.addComment(comment);
					json = this.setJson(true, "系统用户评论成功", "发送成功");
				}
			}
			else{
				// 登陆用户id
				...
				// 添加评论
				commentService.addComment(comment);
				json = this.setJson(true, "用户评论成功", "发送成功");
			}
		} catch (Exception e) {
