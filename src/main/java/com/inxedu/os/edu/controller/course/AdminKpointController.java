...
public class AdminKpointController extends BaseController {
    ...
    public Map<String,Object> updateKpointParentId(HttpServletRequest request,@PathVariable("parentId") int parentId,@PathVariable("kpointId") int kpointId){
     ...
     */
    @RequestMapping("/kpoint/deletekpoint/{kpointId}")
    @ResponseBody
    public Map<String,Object> deleteKpoint(HttpServletRequest request,@PathVariable("kpointId") int kpointId){
    	Map<String,Object> json = new HashMap<String,Object>();
    	try{
    		CourseKpoint courseKpoint = courseKpointService.queryCourseKpointById(kpointId);
    		Course course = courseService.queryCourseById(courseKpoint.getCourseId());
    		int userId = Integer.valueOf(SingletonLoginUtils.getLoginSysUserId(request));
    		if(course.getUserId() == userId || userId == 1){
    			courseKpointService.deleteKpointById(kpointId);
        		json = this.setJson(true, null, null);
    		}
    		else{
    			json = this.setJson(false, "没有此权限！", null);
    ...
    public Map<String,Object> updateKpoint(HttpServletRequest request,@ModelAttribute("courseKpoint") CourseKpoint courseKpoint){
    	Map<String,Object> json = new HashMap<String,Object>();
    	try{
    		CourseKpoint ck = courseKpointService.queryCourseKpointById(courseKpoint.getKpointId());
    		Course course = courseService.queryCourseById(ck.getCourseId());
    		int userId = Integer.valueOf(SingletonLoginUtils.getLoginSysUserId(request));
    		if(course.getUserId() == userId || userId == 1){
    ...
    @ResponseBody
    public Map<String,Object> queryCourseKpointById(HttpServletRequest request,@PathVariable("kpointId") int kpointId){
    	Map<String,Object> json = new HashMap<String,Object>();
    	try{
    		CourseKpoint courseKpoint = courseKpointService.queryCourseKpointById(kpointId);
    		Course course = courseService.queryCourseById(courseKpoint.getCourseId());
    ...
    @ResponseBody
    public Map<String,Object> addKpoint(HttpServletRequest request,@ModelAttribute("courseKpoint") CourseKpoint courseKpoint){
    	Map<String,Object> json = new HashMap<String,Object>();
    	try{
    		Course course = courseService.queryCourseById(courseKpoint.getCourseId());
    		int userId = Integer.valueOf(SingletonLoginUtils.getLoginSysUserId(request));
