...
public class UserServiceImpl implements UserService{
	...
	public boolean checkNumber(String number){
		int count = userDao.checkNumber(number);
		if(count>0){
			return true;
		}
		return false;
	...
	public int queryUserIdByStudentNumber(String number){
		return userDao.queryUserIdByStudentNumber(number);
	}
	
	public int getClassNumber(int classId){
		return userDao.getClassNumber(classId);
	}
	...
	public int queryUserCountByClass(int clazzId){
		return userDao.queryUserCountByClass(clazzId);
	}
	...
	public String updateImportExcel(HttpServletRequest request,MultipartFile myFile,Integer mark,int clazzId) throws Exception {
		String msg="";
		...
		for (int i = 1; i <= rows+1; i++) {
			...
			if (row != null) {
				// **读取cell**
				String number = getCellValue(row.getCell((short) 0));//学号
				String name = getCellValue(row.getCell((short) 1));//姓名
				String pwd = getCellValue(row.getCell((short) 2));//密码
				...
				if(ObjectUtils.isNull(number) || number.equals("")){
					if(mark==1){
						...
						throw new BaseException("第"+i+"行学号错误<br/>");
					}
				}
				if(ObjectUtils.isNotNull(number) && StringUtils.isNotEmpty(number)){
					if(!(number.matches(CommonConstants.numberRegex))){
						if(mark==1){
							msg+="第"+i+"行学号格式错误<br/>";
							continue;
						}else{
							throw new BaseException("第"+i+"行学号格式错误<br/>");
						}
					}
				}
				boolean d = checkNumber(number);
				if (d==true) {
					if(ObjectUtils.isNotNull(pwd) && !pwd.equals("")){
						if(pwd.length()<6 || pwd.length()>20){
							...
							int userId = queryUserIdByStudentNumber(number);
					...
					User user = new User();
					user.setStudentNumber(number);//用户学员学号
