...
function isLogin2(){
	var is = false;
	var sysUser = getLoginSysUser();
	if(sysUser!=null && sysUser.userId>0){
		is=true;
	}
	return is;
}
...
function getLoginSysUser(){
	var sysUser = null;
	$.ajax({
		url:baselocation+'/uc/getloginSysUser',
		type:'post',
		async:false,
		dataType:'json',
		success:function(result){
 ...
 * @returns true false
 */
function checkTeacher(courseId){
	var flag = false;
	$.ajax({
		url:baselocation+'/uc/checkTeacher/'+courseId,
		type:'post',
		async:false,
		dataType:'json',
		success:function(result){
