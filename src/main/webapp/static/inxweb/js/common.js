...
function cardChange(oTitle, oCont, current, callback) {
    ...
    oTitle.click(function() {
        _index = oTitle.index(this);//序号
        $(this).addClass(current).siblings().removeClass(current);//设置该标签为当前标签，并取消其他标签的current
        oCont.eq(_index).show().siblings().hide();//显示当前标签的内容并隐藏其他标签的内容
        if(typeof callback === "function") {callback();};//执行函数
    }).eq(0).click();//默认显示第一个标签
}
...
*/
function lrFun(type) {
    var oBg = $('<div class="bMask"></div>').appendTo($("body")),//背景变暗
        //登录、注册和关闭的头部标签
                ...
                '<section class="dis e-login-ele">'+
                    ...
                    '<div>'+
                        '<ol class="e-login-options">'+
                            '<li>'+
                                '<input id="u-email" type="text" placeholder="请输入登录邮箱/手机号/学号"  name="" value="" onkeyup="$(this).next().html(\'\');">'+
...
//                '</section>'+
    ...
    $("#lrEleWrap").html(rlEle[0]);//把内容添加进头部标签
    placeholderFun();//placeholder兼容IE方法
    var dTop = (parseInt(document.documentElement.clientHeight, 10)/2) + (parseInt(document.documentElement.scrollTop || document.body.scrollTop, 10)),//屏幕中间位置的高度
        dH = dialogEle.height(),//高度
        dW = dialogEle.width(),//宽度
        dHead = $(".dialog-ele>h4");
    dialogEle.css({"top" : (dTop-(dH/2)) , "margin-left" : -(dW/2)});//设置位置
    dHead.css({"width" : (dW-"12")}); //设置宽度;
    //将头部标签的点击事件和内容标签绑定
    cardChange("#d-s-head-tab>a", "#d-s-head-cont>section", "current", function() {
    	var dH = dialogEle.height();
    	dialogEle.css({"top" : (dTop-(dH/2)) , "margin-left" : -(dW/2)});
    });
    $(".dClose").bind("click", function() {dialogEle.remove();oBg.remove();});//关闭时关闭弹出框
    //如果是注册，则默认显示注册窗口
	if(type==2){
...
function placeholderFun() {
  ...
  if(!supportPlaceholder){
  	$("input").not("input[type='password']").each(//把input绑定事件 排除password框  
          function(){  
              if($(this).val()=="" && $(this).attr("placeholder")!=""){  //value为空，placeholder不为空时
                  $(this).val($(this).attr("placeholder"));  //给value赋予placeholder的初始值
                  //点击该框时，若value为初始值，则清空
                  $(this).focus(function(){  
                      if($(this).val()==$(this).attr("placeholder")) $(this).val("");  
                  }); 
                  //离开该框时，若value为空，则赋予初始值
                  $(this).blur(function(){  
      ...
      //对password框的特殊处理
      var pwdField    = $("input[type=password]");//密码输入框
      pwdField.each(function() {
    	  var _this = $(this),//密码标签
    	   	  index = _this.index(),//序号
    	      pwdVal = _this.attr('placeholder');//初始值
    	  _this.after('<input id="pwdPlaceholder'+index+'" type="text" value='+pwdVal+' autocomplete="off" />');//添加密码初始标签
    	  var pwdFieldColn = _this.next();  
    	  pwdFieldColn.show();  //密码初始标签显示
          _this.hide();//密码标签隐藏
          //点击密码初始标签时，密码初始标签隐藏，密码标签显示
          pwdFieldColn.focus(function(){  
        	  pwdFieldColn.hide();  
              _this.show();  
              _this.focus();  
          });  
          //离开密码标签时，若密码为空，则显示密码初始标签
          _this.blur(function(){  
              if(_this.val() == '') {  
            	  pwdFieldColn.show();  
                  _this.hide();  
...
function dialogLogin(type){
	var userName=$("#u-email").val();//用户名
    var pwd = $("#u-password").val();//密码
    var autoThirty=$("#autoThirty").prop("checked")//是否自动登录
    $("#u-email").next().html('');//清空账号信息
    $("#u-password").next().html('');//清空密码信息
    $(".e-l-jy").html('');//清空登录信息
    //判断账号是否为空
    if(userName==""||userName==null){
    	$("#u-email").next().html('<span class="c-orange"><em class="icon16 u-a-cw">&nbsp;</em>请输入正确的登录账号！</span>');
        return false;
    }
    //判断密码是否为空
    if(pwd==""||pwd==null){
	...
	$.ajax({
		...
		success:function(result){
			if(result.success==false){
				$(".e-l-jy").html('<font class="fsize12 c-orange">'+result.message+'</font>');//显示失败信息
			}else{
				if(type==1){
					window.location.href="/uc/index";//跳转到个人中心
				}else{
...
//	$(".e-l-jy").html('');//清空提示消息
//	var emailVal=$("#u-email-reg").val();//获取邮箱
...
//	};
//	
//	var mobileVal=$("#u-mobile-reg").val();//获取手机
...
//		return;
//	}
//	if(($("#u-password-reg").val()).indexOf(" ")!=-1){//密码是否包含空格
...
//			}else {
//				$(".e-l-jy").html('<font class="fsize12 c-orange">'+result.message+'</font>');//失败则显示信息
//			}
//		},
//		error : function(error) {
				...
				if(resultList.length!=0){
					var useImg="";
	 				...
	 				for(var i=0;i<resultList.length;i++){
	 					...
	 					userShowName=resultList[i].userName;
	 					if(userShowName==null || $.trim(userShowName)==''){
	 						userShowName = resultList[i].studentNumber;
	 					}
