				...
				<li>
					...
					<div class="of">
						...
						<font class="fsize12 c-blue"> 
							<c:if test="${empty parentComment.name }">
							<c:if test="${empty parentComment.studentNumber || parentComment.studentNumber==''}">
								<font color="red">${parentComment.sysUserName}（管理员）</font>
							</c:if>
							<c:if test="${!empty parentComment.studentNumber && parentComment.studentNumber!='' }">
								${parentComment.studentNumber}
							</c:if>
							</c:if>
							<c:if test="${!empty parentComment.name }">
								${parentComment.name}
								...
								<c:forEach items="${commentList }" var="ac" varStatus="index">
									<dd>
										...
										<div class="of">
											...
											<c:if test="${empty ac.name }">
									        <c:if test="${empty ac.studentNumber || ac.studentNumber==''}">
										        <font color="red">${ac.sysUserName}（管理员）</font>
									        </c:if>
									        <c:if test="${!empty ac.studentNumber && ac.studentNumber!='' }">
										        ${ac.studentNumber}
									        </c:if>
