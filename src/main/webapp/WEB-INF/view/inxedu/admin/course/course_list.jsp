...
function deleteCourse(courseId){
	...
	$.ajax({
		url:baselocation +'/admin/cou/deleteCourse/'+courseId,
		type:'post',
		dataType:'json',
...
function copyCourse(courseId){
	...
	$.ajax({
		url:baselocation +'/admin/cou/copyCourse/'+courseId,
		type:'post',
		dataType:'json',
		success:function(result){
			if(result.success==false){
			...
			<tbody>
				<c:forEach items="${courseList}" var="course">
					<tr class="odd">
						...
						<c:if test="${course.userId == currentSysUser.userId or currentSysUser.roleId == 1}">
						    <td align="center">
							    ...
							    <a href="javascript:void(0)" onclick="deleteCourse(${course.courseId})" class="button tooltip">删除</a>
							    <a href="${ctx}/admin/cou/lookcoursehistory/${course.courseId}" class="button tooltip">查看观看历史</a>
							    <a href="javascript:void(0)" onclick="copyCourse(${course.courseId})" class="button tooltip">复刻</a>
							    <a href="${ctx}/admin/test/getTestList/${course.courseId}" class="button tooltip">查看测验</a>
						    </td>
						</c:if>
						<c:if test="${course.userId != currentSysUser.userId and currentSysUser.roleId != 1}">
						    <td align="center">
						        <a href="javascript:void(0)" onclick="copyCourse(${course.courseId})" class="button tooltip">复刻</a>
						    </td>
						</c:if>
